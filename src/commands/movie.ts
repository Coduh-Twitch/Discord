...
export const truncate = (input, length: number = 5) =>
  input.length > length ? `${input.substring(0, length)}...` : input;

export function formatMovieString(
  movie: Partial<TMDBMovieFull>,
  pretty: boolean = true,
  trunc: boolean = false,
  length: number = 50,
): string {
...
export async function getMovieById(
  movieId: string,
  guildId: string,
): Promise<TMDBMovieFull | null> {
  try {
    let res = await axios.get(`${process.env.TMDB_API_URL}/movie/${movieId}`, {
...
async function getCreditById(creditId: string): Promise<TMDBCreditFull | null> {
  let res = await axios.get(`${process.env.TMDB_API_URL}/credit/${creditId}`, {
    headers: { Authorization: `Bearer ${process.env.TMDB_TOKEN}` },
  });
  return res.data || null;
}

async function getDirectorsByMovieId(
  movieId: string,
): Promise<TMDBCrewMember[]> {
  let crew = await getCrewByMovieId(movieId);
  if (!crew) return [];
  return crew.filter((c) => c.job === "Director");
}

export async function buildMovieContainer(
  movie: TMDBMovieFull,
  ...
  if (movie.backdrop_path && withImages) {
    movieContainer.addMediaGallery([
      {
        media: { url: `https://image.tmdb.org/t/p/w780${movie.backdrop_path}` },
      },
    ]);
  }

  if (movie.poster_path) {
    movieContainer.addThumbnailAccessorySection(
  ...
  movieContainer.addTextDisplay(
    `${blockQuote(movie.overview || "No Overview Found")}`,
  );
  movieContainer.addTextDisplay(
...
export async function sendMoviePoll(
  ...
  duration: 1 | 4 | 8 | 24 | 72 = 24,
) {
  let movies: TMDBMovieFull[] = [];

  let mvPromises = Promise.all(
    movieIds
      .filter((v) => v !== null)
      .map(async (v) => {
        if (v) {
  ...
  let promises = Promise.all(
    Object.values(movieIds).map(async (v) => {
      if (v) {
        let movie = await getMovieById(v, guildId);
        if (movie)
          return { text: `${formatMovieString(movie, true, true, 45)}` };
      }
    }),
  );
  ...
  containers.forEach((c, i) => {
    channel
      .send({ components: [c], flags: [MessageFlags.IsComponentsV2] })
      .then((m) => {
        sent += 1;
    ...
    if (sent === poll.answers.length && !polled) {
      ...
      channel
        ...
        .catch((e) => {
          if (interaction)
            interaction.editReply({
              content: `Something went wrong: ${e?.message || "No error message"}`,
            });
        });
    }
  }, 1e3);
...
export async function getContentWarningUrl(
  movie: Partial<TMDBMovieFull>,
  getId: boolean = false,
): Promise<string | null> {
  ...
  if (!movie.imdb_id) {
    ...
    res = await axios.get(
      `${process.env.DOG_API_URL}/dddsearch?q=${encodeURIComponent(`${movie.title}`)}`,
      {
        headers: {
    ...
    console.log(
      `Searching for movie ${movie.title} with IMDB id ${movie.imdb_id}`,
    );
    res = await axios.get(
      `${process.env.DOG_API_URL}/dddsearch?imdb=${movie.imdb_id}`,
      {
        headers: {
          "X-API-KEY": process.env.DOG_API_KEY,
          Accept: "application/json",
  ...
  if (!res || !res.data || !res.data?.items) {
    ...
    let firstResult =
      results.find((r) => r.tmdbId !== null && r.tmdbId === movie.id) ||
      results[0];
...
export async function getFlashingLightWarning(
  movie: Partial<TMDBMovieFull>,
): Promise<{ flash: boolean; comment: string | null }> {
  let def = { flash: false, comment: null };
  let contentWarningId = await getContentWarningUrl(movie, true);
  if (!contentWarningId) return def;

  let res = await axios.get(
    `${process.env.DOG_API_URL}/media/${contentWarningId}`,
    {
      headers: {
        "X-API-KEY": process.env.DOG_API_KEY,
        Accept: "application/json",
  ...
  if (yes > no) {
    let comment =
      flashingTopic.comments.sort((a, b) => b.voteSum - a.voteSum)?.[0]
        ?.comment || null;
...
const MovieCommand: Command = {
  ...
  autocomplete: async (interaction: AutocompleteInteraction) => {
    ...
    if (
      focusedOption.name === "query" ||
      focusedOption.name.startsWith("movie-")
    ) {
      ...
      if (!query || query === "")
        results = [
          { name: "Please start typing your search query.", value: "none" },
        ];
      console.log(query);
      ...
      let { data: res } =
        query !== ""
          ? await axios.get(
              `${process.env.TMDB_API_URL}/search/movie?query=${encodeURIComponent(query)}&region=us`,
              {
                headers: { Authorization: `Bearer ${process.env.TMDB_TOKEN}` },
              },
            )
      ...
      if ((!res || !res?.results) && query !== "")
        results = [
          {
            name: "No results found, please try another query.",
            value: "none",
          },
        ];
  ...
  run: async (interaction: ChatInputCommandInteraction) => {
    ...
    if (subcommand === "search") {
      ...
      if (!movie)
        return interaction.editReply({
          content: `Movie ID ${movieId} not found.`,
        });
      ...
      interaction.editReply({
        ...
        components: [movieContainer.buildContainer()],
      });
    }
    ...
    if (subcommand === "create-event") {
      ...
      async function movieContainerExt(
        with_interaction: boolean = false,
        ...
        if (with_interaction) {
          ...
          movieContainer.addButtonAccessorySection(
            `Create event and send announcement? ->`,
            ButtonStyle.Success,
            "Yes",
            parseCustomId(generateCustomId(interaction, `correct-movie`)),
          );
          movieContainer.addButtonAccessorySection(
            `Cancel your request? ->`,
            ButtonStyle.Danger,
            "Cancel",
            parseCustomId(generateCustomId(interaction, `cancel-movie`)),
          );
          movieContainer.addSeparator();
        }
        ...
        if (!expired && with_interaction) {
          movieContainer.addTextDisplay(
            `-# This interaction expires <t:${Math.floor((Date.now() + interactionTimeout) / 1000)}:R>`,
          );
        } else if (expired) {
          movieContainer.addSeparator();
      ...
      interaction
        .editReply({
          flags: [MessageFlags.IsComponentsV2],
          components: [(await movieContainerExt(true, false)).buildContainer()],
        })
        .then(async (m) => {
          if (!response.resource?.message) return;
          let buttonPress: ButtonInteraction | null;
          buttonPress = await response.resource.message
          ...
          if (!buttonPress) {
            if (interaction.replied)
              interaction.editReply({
                components: [
                  (await movieContainerExt(false, true)).buildContainer(),
                ],
              });
          } else {
            ...
            if (buttonPress.customId.includes("correct")) {
              let movieStage = interaction.guild.channels.cache.get(
              ...
              if (movie.backdrop_path)
                eventData.image = `https://image.tmdb.org/t/p/w780${movie.backdrop_path}`;

              let modal = new ModalBuilder()
                .setTitle("Enter Start Time")
                .addTextDisplayComponents([
                  TMComponentBuilder.textDisplay(`## Having Trouble?`),
                  TMComponentBuilder.textDisplay(
                    `Head to [hammertime.cyou](https://hammertime.cyou) to get a timestamp, then paste it here.\n\n**Copy the timestamp that consists of numbers only. (</> icon)**`,
                  ),
                ])
                .setCustomId("modal");
              let label = new LabelBuilder()
                .setLabel("Timestamp")
                .setDescription("Please enter the event start timestamp")
                .setTextInputComponent(
                  new TextInputBuilder()
                    .setCustomId("modal-timestamp")
                    .setRequired(true)
                    .setPlaceholder(`${Date.now()}`)
                    .setMinLength(`${Math.floor(Date.now() / 1000)}`.length)
                    .setStyle(TextInputStyle.Short),
                );
              modal.addLabelComponents(label);
              modal.addTextDisplayComponents(
                TMComponentBuilder.textDisplay(
                  `This interaction expires <t:${Math.floor((Date.now() + modalTimeout) / 1000)}:R>`,
                ),
              );
              ...
              modalSubmit = await buttonPress
                .awaitModalSubmit({ time: modalTimeout })
                .catch((e) => null);
              ...
              if (!modalSubmit) {
                ...
                let timestamp =
                  modalSubmit.fields.getTextInputValue("modal-timestamp");
                ...
                interaction.guild.scheduledEvents
                  ...
                  .then(async (event) => {
                    modalSubmit.reply({
                      flags: [MessageFlags.Ephemeral],
                      content: `Successfully created event to begin <t:${Math.floor(event.scheduledStartTimestamp / 1000)}:R>`,
                    });
                    if (interaction.replied)
                      interaction.editReply({
                        components: [
                          (
                            await movieContainerExt(false, false)
                          ).buildContainer(),
                        ],
                      });
                    if (announcementChannel.isSendable()) {
                      announcementChannel
                        .send({
                          ...
                          components: [
                            (
                              await buildMovieContainer(movie, true)
                            ).buildContainer(),
                          ],
                        })
                        .then((posterMsg) => {
    ...
    if (subcommand === "poll") {
      ...
      let multiselect = interaction.options.getBoolean(
