  ...
  run: async (interaction) => {
    if (rollers.has(interaction.user.id)) {
      await interaction.reply({
        content: `${await appEmoji(interaction.client, "timej")} You're already using the slot machine. Don't be like my Grandma, you can't gamble in prison!`,
    ...
    if (Number.isNaN(Number(amount)) && amount === "all")
      ...
      return interaction.editReply({
        ...
        components: [
          new TMComponentBuilder()
            ...
            .addTextDisplay(
              `## ${await appEmoji(interaction.client, "riotj")} What are You Doing?\n${await appEmoji(interaction.client, "what")} You're trying to give me fake money, DO NOT REDEEM IT, DO NOT REDEEM IT!`,
            )
            .buildContainer(),
        ],
      });
      ...
      return interaction.editReply({
        ...
        components: [
          new TMComponentBuilder()
            .setAccentColor(Colors.Red)
            .addTextDisplay(
              `## ${await appEmoji(interaction.client, "nono")} Not Enough ${config.point_name()}s\nYou need at least 50 ${config.point_name(true)}s to gamble, soldier!`,
            )
            .buildContainer(),
        ],
      });
    ...
    async function rollContainer(
      ...
      if (previousRoll.length > 0) {
        ...
        let rt = "Rolling...";
        if (savedR1 && savedR3)
          rt = `${rt} ${await appEmoji(interaction.client, "gigachad")}`;
        ...
        if (rolling)
          container.addTextDisplay(
            `## ${await appEmoji(interaction.client, "pausej")} ${rt}`,
          );
        if (!rolling) {
          if (!win && !jackpot) {
            console.log("LOSER");
            container.addTextDisplay(
              `## ${userMention(interaction.user.id)} lost ${amount.toLocaleString()} ${config.point_name(true)}s ${await appEmoji(interaction.client, "sadgge")}`,
            );
        ...
        container.addTextDisplay(
          `# | ${(await Promise.all(previousRoll.map(async (e) => `${await appEmoji(interaction.client, e)}`))).join(" | ")} |`,
        );
      } else {
        if (rolling)
          container.addTextDisplay(
            `## ${await appEmoji(interaction.client, "typej")} Setting up the machine...`,
          );
      }
    ...
    await interaction.editReply({
      flags: [MessageFlags.IsComponentsV2],
      components: [(await rollContainer(true, [])).buildContainer()],
    });
    ...
    interval = setInterval(async () => {
      if (rolls < 3) {
        ...
        let rand2 =
          savedR2 || emotes[Math.floor(Math.random() * emotes.length)];
        let rand3 =
        ...
        await interaction.editReply({
          components: [
            (await rollContainer(true, [rand1, rand2, rand3])).buildContainer(),
          ],
        });
        rolls += 1;
      } else {
        await interaction.editReply({
          components: [(await rollContainer(false, [])).buildContainer()],
        });
        savedR1 = null;
        savedR2 = null;
        savedR3 = null;
