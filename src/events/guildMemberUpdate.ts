...
export default {
  ...
  run: async (old_member: GuildMember, new_member: GuildMember) => {
    if (
      !old_member.roles.cache.has(config.roles.members) &&
      new_member.roles.cache.has(config.roles.members)
    ) {
      ...
      const introChannel: TextChannel = member.guild.channels.cache.get(
        config.channels.joins,
      ) as TextChannel;
      const hangoutChannel: TextChannel = member.guild.channels.cache.get(
        config.channels.hangout,
      ) as TextChannel;
      const con = new TMComponentBuilder().setAccentColor(
        (await avg(member.displayAvatarURL())) as number,
      );
      ...
      con.addThumbnailAccessorySection(
        `## ${await appEmoji(new_member.client, emojis?.[random] || emojis[0])} Welcome to coduh's crib, ${userMention(new_member.id)}!\n-# Joined <t:${Math.floor(member.joinedTimestamp / 1000)}:R> ∙ Account Created <t:${Math.floor(member.user.createdTimestamp / 1000)}:R>\n\n**Check out the most common channels for new members!**`,
      ...
      try {
        await post(`${process.env.CHATBOT_BASE_URL}/api/discord/new-member`, {
          username: new_member.user.username,
          memberCount: new_member.guild.memberCount,
        });
      } catch (e) {
